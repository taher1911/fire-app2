<template>
  <div class="home">
    <div class="button">
      <button class="adminMainButton" @click="createData()">
        添加客户单位
      </button>
    </div>
    <div class="content">
      ...
      <div v-for="d in clientsData" :key="d.id" class="item">
        ...
        <span class="acc-num">{{ d.acc_num }}</span>
...
.home {
  ...
  .content {
    ...
    .items-titles {
      padding: 15px 10px;
      display: grid;
      grid-template-columns: 1fr 1.2fr 3fr 8fr;
      font-size: 17px;
      color: #ffffff;
      font-weight: 500;
    }
    .item {
      padding: 15px 10px;
      display: grid;
      grid-template-columns: 1fr 1.2fr 3fr 7fr 1.1fr;
      transition: all 0.3s;
      &:hover {
